...
func ReportRoutes(
	...
	reports.Get("/statistics", func(c *fiber.Ctx) error {
		...
		// ID dan Role sudah dijamin ada oleh jwtMiddleware yang dipasang pada grup 'reports'
		userIDStr, ok := c.Locals("userID").(string)
		userRole, roleOk := c.Locals("role").(string)

		if !ok || !roleOk || userIDStr == "" || userRole == "" {
			// Status 401: Data otentikasi hilang, meskipun melewati JWT.
		...
		// --- 2. Panggil Service (Service akan menangani logika RBAC dan filtering) ---
		result, err := achievementSvc.GetAchievementStatistics(ctx, userRole, parsedUserID)
		
		if err != nil {
			// Penanganan error dari Service Layer
			...
			if strings.Contains(errorMessage, "forbidden") {
				...
				// Mengembalikan pesan error yang spesifik dari service (misal: "user is not a registered student")
			}
